...
namespace GDD3400.Labyrinth
{
    ...
    public class EnemyAgent : MonoBehaviour
    {
        ...
        private EnemyStates currState = EnemyStates.Wander;
        ...
        private const float _seenTime = 5f;
        ...
        private void Perception()
        {
            ...
            else
            {
                ...
                if (_playerSeen) { _seenTimer += Time.deltaTime; }
                if (_seenTimer >= _seenTime) { _seenTimer = 0; _playerSeen = false; currState = EnemyStates.Wander; }
            }

            Debug.DrawRay(rayOrigin, rayDir * _SightDistance, Color.red);
        }
        ...
        public void SetDestinationTarget(Vector3 destination)
        {
            ...
            // If straight line distance is greater than minimum required, let's do pathfinding!!!
            if (Vector3.Distance(transform.position, destination) > _MinimumPathDistance)
            {
                PathNode startNode = _levelManager.GetNode(transform.position),
                    endNode = _levelManager.GetNode(destination);
